...
class SearchPageQuotes(unittest.TestCase):
        ...
        for current_search_term in searchTerms:
            ...
            while(count<5):
                if sel.is_element_present(search_page_obj.next_page_link):
                    ...
                    self.failUnless(sel.is_element_present(search_page_obj.result_div), "No results for %s on page # %s" %(current_search_term,(count+1)))
                    count = count + 1
                else:
                    break
