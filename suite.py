...
for param in cmd_params:
    if param[1:2] == 'U':
        if not 'http://' in str(param[2:]):
            ...
            vars.ConnectionParameters.baseurl = str(param[2:])
            vars.ConnectionParameters.authurl = str(param[2:])
        print "This suite runs on %s" %(vars.ConnectionParameters.baseurl)
        vars.ConnectionParameters.authurlssl = vars.ConnectionParameters.authurl.replace("http", "https")
        vars.ConnectionParameters.baseurl_ssl = vars.ConnectionParameters.baseurl.replace("http", "https")
